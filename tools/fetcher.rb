...
begin
  ...
  File.open('links.txt', 'w') do |f|
    db.each do |id, record|
      if record["streamtypes"].include?("mp4")  then
        f.print "you-get -F mp4 -o /Volumes/Backup/TEDxTalks ", record["link"], "\n"
        # else
        # f.print "you-get -o /Volumes/Backup/TEDxTalks ", record["link"], "\n"
      end
    end
  end
end
